...
RETURNING id, name`, brewery.Name).Scan(&brewery.ID, &brewery.Name)
	if err != nil {
		return models.Brewery{}, fmt.Errorf("%w: %v", errorcrud.ErrCreatingData, err)
	}
	return *brewery, nil
}
...
RETURNING id, name`, brewery.Name, brewery.ID).Scan(&brewery.ID, &brewery.Name)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return models.Brewery{}, fmt.Errorf("%w: %v", errorcrud.ErrBreweryNotFound, err)
		}
		return models.Brewery{}, fmt.Errorf("%w: %v", errorcrud.ErrUpdatingData, err)
	}
	return *brewery, nil
}
